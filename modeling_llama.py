...
None].float().expand(position_ids.shape[0], -1, 1).to(x.device)
        position_ids_expanded = position_ids[:, None, :].float()

        device_type = x.device.type if isinstance(x.device.type, str)
and x.device.type != "mps" else "cpu"
...
#
#    # 1) 원본 FP32 어텐션 계산
#    A_fp32 = torch.matmul(orig_query, orig_key_states.transpose(2,
3)) * scaling
#
...
#    np.save(os.path.join(dequant_query_dir,
f"q8_matmul_layer{layer_idx}_step{counter_q}.npy"),
#            A_qdq.cpu().numpy())
#    print(f"[SAVE QDQ] layer={layer_idx} step={counter_q},
shape={A_qdq.shape}")
#
#    # 3) SNR 계산
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    send_tensor_k_with_len(k_int8, uart, tag=b'K')
    #####################
    ...
    print("→ abs error     :", (orig_q[0, head_idx, 0].float() -
dequant_q).abs().cpu().numpy())
    ...
    print("→ abs error     :", (orig_k[0, head_idx, 0].float() -
dequant_k).abs().cpu().numpy())

    # 저장
    if step % SAVE_INTERVAL == 0:
        os.makedirs("./debug_quant", exist_ok=True)
        np.save(f"./debug_quant/query_int8_layer{layer_idx}_step{step}.npy",
q_int8.cpu().numpy())
        np.save(f"./debug_quant/query_scale_layer{layer_idx}_step{step}.npy",
q_scale.cpu().numpy())
        np.save(f"./debug_quant/key_int8_layer{layer_idx}_step{step}.npy",
k_int8.cpu().numpy())
        np.save(f"./debug_quant/key_scale_layer{layer_idx}_step{step}.npy",
k_scale.cpu().numpy())
    ...
    #attn_weight =
torch.tensor(fpga_result_float,device=orig_q.device,
dtype=torch.float32)
    #attn_weight = attn_weights.view(1, module.num_heads,
orig_q.size(2), orig_k.size(2)
    #if attention_mask is not None:
    #    cm = attention_mask[:, :, :, : key_states.shape[-2]]
    #    attn_weights = attn_weights + cm
    #attn_weight = torchc.nn.functional.sofmax(attn_weight, dim = -1)
    #attn_weight = torch.nn.functional.dropout(attn_weight, p =
dropout, training=module.training)
...
def quantize_tensor(tensor, bits=4):
    ...
    for h in range(H):
        ...
        abs_max = torch.quantile(head_tensor.abs().float(), 0.995)  #
99.5% 구간만 고려
...
#    tensor_file = os.path.join(save_dir,
f"{name}_layer{layer_idx}_step{step}_tensor.npy")
#    meta_file = os.path.join(save_dir,
f"{name}_layer{layer_idx}_step{step}_meta.npy")

#   print(f"[SAVE QUANT] {tensor_file}, shape: {quant_tensor.shape},
scale={scale:.4f}, zp={zp}")
#   np.save(tensor_file, quant_tensor.cpu().numpy().astype(np.int8))
#   np.save(meta_file, {"scale": float(scale), "zero_point": int(zp)},
allow_pickle=True)
#    print(f"[SAVE QUANT] {tensor_file}, shape: {quant_tensor.shape}")
#
...
#        np.save(meta_file, {"scale": scale, "zero_point": int(zp)},
allow_pickle=True)
#    else:
#        #print(f"  scale={scale:.6f}, zp={zp}")
#        np.save(meta_file, {"scale": float(scale), "zero_point":
int(zp)}, allow_pickle=True)
#
#    np.save(tensor_file, quant_tensor.cpu().numpy().astype(np.int8))
################################################################################
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.head_dim = getattr(config, "head_dim", config.hidden_size
...
self.head_dim, bias=config.attention_bias
        )
...
###############################################################################################
        ...
        self.q_proj = bnb.nn.Linear8bitLt(config.hidden_size,
config.hidden_size, bias = True )

        self.k_proj = bnb.nn.Linear8bitLt(config.hidden_size,
config.hidden_Size, bias = True)

        self.v_proj = bnb.nn.Linear8bitLt(config.hidden_size,
config.hidden_Size, bias = True)

        self.o_proj = nn.Linear8bitLt(config.hidden_size, config.hidden_Size)
...
## TY code start
##################################################################################################
        #np.save("query_states.npy",
query_states.to(torch.float32).cpu().numpy()) #TY
        #np.save("key_states.npy",
key_states.to(torch.float32).cpu().numpy()) #TY

        #save_tensor(query_states, "query", self.layer_idx)
        #save_tensor(key_states, "key", self.layer_idx)

        #q_quant, q_scale, q_zp = quantize_tensor(query_states, bits = 4)
        #k_quant, k_scale, k_zp = quantize_tensor(key_states, bits = 4)

        #save_quantized_tensor(query_states, "query", self.layer_idx)
        #save_quantized_tensor(key_states, "key", self.layer_idx)

        #print("q_quant:", q_quant.shape, "dtype:", q_quant.dtype) #TY
        #print("k_quant:", k_quant.shape, "dtype:", k_quant.dtype) #TY

        #print("q_scale:", q_scale.item())
        #print("q_zero_point:", q_zp.item())
        #print("k_scale:", k_scale.item())
        #print("k_zero_point:", k_zp.item())
        #print(f"[Layer {self.layer_idx}] Query shape:
{query_states.shape}, Quantized shape: {q_quant.shape}")
        #print(f"[Layer {self.layer_idx}] Scale: {q_scale.item():.6f},
ZeroPoint: {q_zp.item():.2f}")
        #print(f"[Layer {self.layer_idx}] Key shape:
{key_states.shape}, Quantized shape: {k_quant.shape}")
        #print(f"[Layer {self.layer_idx}] Scale: {k_scale.item():.6f},
ZeroPoint: {k_zp.item():.2f}")
...
cache_kwargs)
       ...
       # print(f"\n[DEBUG] LlamaAttention layer {self.layer_idx} -
...
`output_attentions=True`. Falling back to "
                    'eager attention. This warning can be removed
using the argument `attn_implementation="eager"` when loading the
model.'
                )
            else:
                attention_interface =
...
layers. Each layer is a [`LlamaDecoderLayer`]
...
checkpointing. Setting `use_cache=False`."
        ...
        # TODO (joao): remove this exception in v4.56 -- it exists for
users that try to pass a legacy cache
        if not isinstance(past_key_values, (type(None), Cache)):
            raise ValueError("The `past_key_values` should be either a
...
past_key_values, output_attentions
    ...
    def _update_causal_mask(
        ...
        # For SDPA, when possible, we will rely on its `is_causal`
argument instead of its `attn_mask` argument, in
...
past_key_values is not None else 0
        ...
        # When output attentions is True, sdpa implementation's
forward method calls the eager implementation's forward
        if self.config._attn_implementation == "sdpa" and not
using_static_cache and not output_attentions:
...
self._prepare_4d_causal_attention_mask_with_cache_position(
        ...
        if (
            ...
            # Attend to all tokens in fully masked rows in the
causal_mask, for example the relevant first rows when
            # using left padding. This is required by
F.scaled_dot_product_attention memory-efficient attention path.
            # Details: https://github.com/pytorch/pytorch/issues/110213
            min_dtype = torch.finfo(dtype).min
            causal_mask =
AttentionMaskConverter._unmask_unattended(causal_mask, min_dtype)
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        **kwargs,
        ...
        """
        Creates a causal 4D mask of shape `(batch_size, 1,
query_length, key_value_length)` from a 2D mask of shape
        `(batch_size, key_value_length)`, or if the input
`attention_mask` is already 4D, do nothing.
...
key_value_length)` or a 4D attention mask of shape
            ...
            target_length (`int`):
                The target length: when generating with static cache,
the mask should be as long as the static cache,
                to account for the 0 padding, the part of the cache
...
tokens in the sequence.
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            # In this case we assume that the mask comes already in
inverted form and requires no inversion or slicing.
...
:].expand(batch_size, 1, -1, -1)
            if attention_mask is not None:
                causal_mask = causal_mask.clone()  # copy to
contiguous memory for in-place edit
...
with labels in `[0, ..., config.vocab_size]`.
            ...
            logits_to_keep (`int` or `torch.Tensor`, *optional*):
                If an `int`, compute logits for the last
`logits_to_keep` tokens. If `0`, calculate logits for all
                `input_ids` (special case). Only last token logits are
needed for generation, and calculating them only for that
                token can save memory, which becomes pretty
significant for long sequences or large vocabulary size.
                If a `torch.Tensor`, must be 1D corresponding to the
indices to keep in the sequence length dimension.
                This is useful when using packed tensor format (single
dimension for batch and sequence length).
...
conscious, but I can talk to you."
        ```"""
        output_attentions = output_attentions if output_attentions is
not None else self.config.output_attentions
        output_hidden_states = (
        ...
        # decoder outputs consists of (dec_features, layer_state,
dec_hidden, dec_attn)
        ...
        # Only compute necessary logits, and do not upcast them to
float if we are not computing the loss
        slice_indices = slice(-logits_to_keep, None) if
isinstance(logits_to_keep, int) else logits_to_keep
...
@add_start_docstrings(
    """
    The LLaMa Model transformer with a sequence classification head on
top (linear layer).

    [`LlamaForSequenceClassification`] uses the last token in order to
do the classification, as other causal models
    (e.g. GPT-2) do.

    Since it does classification on the last token, it requires to
know the position of the last token. If a
    `pad_token_id` is defined in the configuration, it finds the last
token that is not a padding token in each row. If
    no `pad_token_id` is defined, it simply takes the last value in
each row of the batch. Since it cannot guess the
    padding tokens when `inputs_embeds` are passed instead of
`input_ids`, it does the same (take the last value in
...
classification/regression loss. Indices should be in `[0, ...,
...
(Cross-Entropy).
        ...
        if self.config.pad_token_id is None and batch_size != 1:
            raise ValueError("Cannot handle batch sizes > 1 if no
padding token is defined.")
        if self.config.pad_token_id is None:
            last_non_pad_token = -1
        elif input_ids is not None:
            # To handle both left- and right- padding, we take the
rightmost token that is not equal to pad_token_id
            non_pad_mask = (input_ids !=
self.config.pad_token_id).to(logits.device, torch.int32)
...
device=logits.device, dtype=torch.int32)
            ...
            last_non_pad_token = -1
            logger.warning_once(
                f"{self.__class__.__name__} will not detect padding
tokens in `inputs_embeds`. Results may be "
                "unexpected if using padding tokens in conjunction
with `inputs_embeds.`"
            )
...
class LlamaForQuestionAnswering(LlamaPreTrainedModel):
    ...
    # Copied from
transformers.models.bloom.modeling_bloom.BloomForQuestionAnswering.__init__
with Bloom->Llama
    def __init__(self, config):
        super().__init__(config)
...
@add_start_docstrings(
    """
    The Llama Model transformer with a token classification head on
top (a linear layer on top of the hidden-states
